"""GlitchTip/Sentry error reporting for the Jura Home Assistant integration."""
...
def init_error_reporting(
    ...
    To opt out, set ``JURA_SENTRY_DSN=""`` or pass ``enabled=False``.

    Args:
        dsn: Sentry DSN override. If None, falls back to JURA_SENTRY_DSN env var,
    ...
    # Resolve DSN: explicit arg > JURA_SENTRY_DSN env var > built-in default
    env_dsn = os.environ.get("JURA_SENTRY_DSN")
...
def _frame_is_jura(frame: dict) -> bool:  # type: ignore[type-arg]
    """True if this stack frame is from the Jura integration or jura lib."""
    module = (frame.get("module") or "") or (frame.get("filename") or "")
    if not module:
        return False
    # Module names: custom_components.jura.*, jura.*
    if module.startswith("custom_components.jura") or module.startswith("jura."):
        return True
    # File paths: .../custom_components/jura/... or .../jura/...
    norm = module.replace("\\", "/")
    return "/custom_components/jura/" in norm or "/jura/" in norm
...
def _scrub_event(event: dict, hint: dict) -> dict | None:  # type: ignore[type-arg]
    ...
    for entry in values:
        ...
        if frames and _frame_is_jura(frames[-1]):
